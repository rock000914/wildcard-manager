...
- 右クリックメニューからカード名のリネーム
- シフトクリックで矩形範囲選択（Windows風の複数選択）
...
- 衣装ごとのプロンプトをまとめて作成
- ダイアログ内でサムネイルプレビュー生成
- LoRA タグの色分け表示（LoRA使用を一目で確認可能）
- タグ削除モード（ボタン色・枠線色で状態がわかりやすく改善）
...
#### 機能追加
- **シフトクリックで矩形範囲選択**: Windowsエクスプローラー風の選択操作。1枚目をクリックし、シフト+クリックで2枚目まで矩形範囲のサムネイルを選択
- **右クリックメニューからカード名リネーム**: サムネイルを右クリックし「名前を変更」で直接リネーム。ファイル名とDBエントリを同期更新
