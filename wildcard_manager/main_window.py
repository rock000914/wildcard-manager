...
class SettingsDialog(QDialog):
    def __init__(self, settings: AppSettings, repo: WildcardRepository, api: ThumbnailApiClient, parent=None):
        ...
        management_form = QFormLayout(management_tab)
        management_form.addRow("保存先 wildcard", self.library_root["widget"])
        management_form.addRow("取込元 wildcard", self.source_wildcard_root["widget"])
        management_form.addRow("取込元 thumbnail", self.source_thumbnail_root["widget"])
        ...
        tabs.addTab(management_tab, "ワイルドカード管理設定")

        thumbnail_tab = QWidget()
        thumbnail_form = QFormLayout(thumbnail_tab)
        thumbnail_form.addRow("保存先 thumbnail", self.thumbnail_root["widget"])
        api_url_row = QWidget()
        ...
        thumbnail_form.addRow("APIベースURL", api_url_row)
        thumbnail_form.addRow("APIタイムアウト", self.api_timeout_sec)
        thumbnail_form.addRow("サムネイルサイズ", self.thumbnail_size)
        ...
        thumbnail_form.addRow("生成 payload JSON", self.generation_extra_payload_json)
        tabs.addTab(thumbnail_tab, "サムネイル設定")

        random_tab = QWidget()
        random_layout = QVBoxLayout(random_tab)
        random_layout.setContentsMargins(12, 12, 12, 12)
        random_layout.setSpacing(8)
        random_note = QLabel(
            "ランダムワイルドカード生成の設定は、ここに集約します。\n"
            "機能実装時にこのタブへ項目を追加していきます。"
        )
        random_note.setWordWrap(True)
        random_layout.addWidget(random_note)
        random_layout.addStretch(1)
        tabs.addTab(random_tab, "ランダム生成設定")
